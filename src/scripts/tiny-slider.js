var tns = (function() {
  ...
  function ea(t, e, n) {
    for (var i in e) {
      var a = ['touchstart', 'touchmove'].indexOf(i) >= 0 && !n && u;
      t.addEventListener(i, e[i], a);
    }
  }
  function na(t, e) {
    for (var n in e) {
      var i = ['touchstart', 'touchmove'].indexOf(n) >= 0 && u;
  ...
  var aa = function(O) {
    ...
    var d = e.tTf ? ki(e.tTf) : Ri(e, 'tTf', $i('transform'), n);
    ...
    var v = e.t3D
      ...
      : Ri(
          ...
          (function(t) {
            ...
            var o = t.length > 9 ? '-' + t.slice(0, -9).toLowerCase() + '-' : '';
            return (
              (o += 'transform'),
    ...
    var x = e.tTDu ? ki(e.tTDu) : Ri(e, 'tTDu', $i('transitionDuration'), n);

    var p = e.tTDe ? ki(e.tTDe) : Ri(e, 'tTDe', $i('transitionDelay'), n);

    var b = e.tADu ? ki(e.tADu) : Ri(e, 'tADu', $i('animationDuration'), n);

    var m = e.tADe ? ki(e.tADe) : Ri(e, 'tADe', $i('animationDelay'), n);

    var C = e.tTE ? ki(e.tTE) : Ri(e, 'tTE', ta(x, 'Transition'), n);

    var w = e.tAE ? ki(e.tAE) : Ri(e, 'tAE', ta(b, 'Animation'), n);

    var M = h.console && typeof h.console.warn === 'function';
    ...
    if (
      ...
      var I = O.mode === 'carousel';
      if (k) {
        0 in k && ((O = Hi(O, k[0])), delete k[0]);
        var A = {};
        for (var N in k) {
          var L = k[N];
          (L = typeof L === 'number' ? { items: L } : L), (A[N] = L);
        }
        (k = A), (A = null);
      }
      if (
        ...
        var B = O.animateDelay;

        var W = O.animateNormal;
      }
      var S;

      var F;

      var q = O.axis === 'horizontal';
      ...
      var ot = vn('fixedWidth');

      var ut = vn('edgePadding');
      ...
      var ft = rt ? 1 : Math.floor(vn('items'));

      var dt = vn('slideBy');

      var vt = O.viewportMax || O.fixedWidthViewportWidth;
      ...
      var gt = vn('autoHeight');

      var xt = vn('controls');

      var bt = vn('controlsText');
      ...
      var Et = vn('autoplayTimeout');

      var At = vn('autoplayText');

      var Nt = vn('autoplayHoverPause');

      var Lt = vn('autoplayResetOnVisibility');

      var Bt = ((at = document.createElement('style')),
      ...
      var Qt = O.preventActionWhenRunning;
      ...
      var ae = {
        ...
        keydown: function(t) {
          ...
          e >= 0 && (e === 0 ? Ee.disabled || fi(t, -1) : Ae.disabled || fi(t, 1));
        }
      };
      ...
      var re = {
        click: function(t) {
          ...
          if (Qi(e, 'data-nav')) {
            ...
            var i = ot || rt ? (n * K) / De : n * ft;

            var a = ve ? n : Math.min(Math.ceil(i), K - 1);
      ...
      var se = { touchstart: Ti, touchmove: Ei, touchend: Ai, touchcancel: Ai };

      var ce = { mousedown: Ti, mousemove: Ei, mouseup: Ai, mouseleave: Ai };

      var fe = dn('controls');

      var de = dn('nav');

      var ve = !!rt || O.navAsThumbnails;
      ...
      var be = O.preventScrollOnTouch === 'force';
      if (fe) {
        var Ce;

        var we;

        var Me = O.controlsContainer;

        var Te = O.controlsContainer ? O.controlsContainer.outerHTML : '';

        var Ee = O.prevButton;

        var Ae = O.nextButton;

        var Ne = O.prevButton ? O.prevButton.outerHTML : '';

        var Le = O.nextButton ? O.nextButton.outerHTML : '';
      }
      if (de) {
        var Be;

        var Se = O.navContainer;

        var Oe = O.navContainer ? O.navContainer.outerHTML : '';
        ...
        var Pe = 'tns-nav-active';

        var ze = 'Carousel Page ';

        var We = ' (Current Slide)';
      }
      if (pe) {
        var Fe;
        ...
        var Qe = O.autoplayDirection === 'forward' ? 1 : -1;

        var Xe = O.autoplayButton;

        var Ye = O.autoplayButton ? O.autoplayButton.outerHTML : '';

        var Ke = ["<span class='tns-visually-hidden'>", ' animation</span>'];
      }
      if (me || he) {
        var Je;
        ...
        (function() {
          ...
          if (k && H) {
            for (var a in k) {
              ...
              var c = rt ? null : vn('items', a);

              var f = vn('fixedWidth', a);

              var d = vn('speed', a);

              var v = vn('edgePadding', a);

              var p = vn('autoHeight', a);
    ...
    function on(t) {
      return (
        (t = t ? Math.max(0, Math.min(yt ? K - 1 : K - ft, t)) : 0), I ? t + Dt : t
      );
    }
    function un(t) {
      for (t == null && (t = qt), I && (t -= Dt); t < 0; ) t += K;
    ...
    function vn(t, e) {
      if ((e == null && (e = J), t === 'items' && ot)) {
        return Math.floor((st + lt) / (ot + lt)) || 1;
      }
      var n = O[t];
      if (k) for (var i in k) e >= parseInt(i) && t in k[i] && (n = k[i][t]);
      return (
        t === 'slideBy' && n === 'page' && (n = vn('items')),
    ...
    function yn(t) {
      ...
      !1 !== t &&
        (e =
          (q ? 'padding-' : 'margin-') + (q ? 'right' : 'bottom') + ': ' + t + 'px;');
      return e;
    }
    function gn(t, e) {
    ...
    function En() {
      ...
      wt && ea(G, se, O.preventScrollOnTouch),
        ...
        gt && (R === 'outer' ? Ut.on('innerLoaded', qn) : $t || qn()),
        Pn(),
        $t ? Hn() : ne && Dn(),
        Ut.on('indexChanged', Vn),
        R === 'inner' && Ut.emit('innerLoaded', Si()),
        typeof Jt === 'function' && Jt(Si()),
        (U = !0);
    }
    function An(t) {
      Oi(function() {
    ...
    function Nn(t) {
      if (U) {
        R === 'outer' && Ut.emit('outerResized', Si(t)), (J = sn());
        var e;
    ...
    function In(t) {
      ...
      if (
        ...
        _.forEach(function(t, e) {
          e < Ht && ((ct || ut) && t <= i + 0.5 && (r = e), a - t >= 0.5 && (n = e));
        });
      } else {
        if (ot) {
          var e = ot + lt;
    ...
    function Vn() {
      Pn(),
        ...
        $n(),
        (function() {
          if (Ct && ((Re = ke >= 0 ? ke : ln()), (ke = -1), Re !== Ie)) {
            var t = Be[Ie];
    ...
    function Yn() {
      ...
      var i = q ? 'right' : 'bottom';

      var a = Y[0].getBoundingClientRect()[n];
      qi(Y, function(t, e) {
    ...
    function ri(t) {
      t == null && (t = ii()), (G.style[Pt] = zt + t + Wt);
    }
    function oi(t, e, n, i) {
      var a = t + ft;
      yt || (a = Math.min(a, Ht));
    ...
    function ui(t, e) {
      It && en(),
        (qt !== jt || e) &&
          ...
          gt && qn(),
          qe && t && ['click', 'keydown'].indexOf(t.type) >= 0 && hi(),
          (Kt = !0),
          nn());
    }
    function li(t) {
    ...
    function si(t) {
      if (I || Kt) {
        if ((Ut.emit('transitionEnd', Si(t)), !I && Ot.length > 0)) {
        ...
        if (
          ...
          R === 'inner' && Ut.emit('innerLoaded', Si()), (Kt = !1), (jt = qt);
        }
      }
    }
    function ci(t, e) {
    ...
    function fi(t, e) {
      ...
      if (ht) {
        if (qt === Vt && e === -1) return void ci('last', t);
        if (qt === Gt && e === 1) return void ci('first', t);
      }
      e &&
        ((qt += dt * e),
        rt && (qt = Math.floor(qt)),
        ui(n || (t && t.type === 'keydown') ? t : null));
    }
    function di() {
    ...
    function Ti(t) {
      ...
      Ut.emit(Ci(t) ? 'touchStart' : 'dragStart', Si(t)),
        !Ci(t) && ['img', 'a'].indexOf(Jn(bi(t))) >= 0 && wi(t),
        (Ze.x = _e.x = e.clientX),
        (Ze.y = _e.y = e.clientY),
    ...
    function Ai(i) {
        ...
        if (Math.abs(a)) {
          ...
          I
            ? (Ue = Oi(function() {
                if (q && !rt) {
                  var t = (-a * ft) / (st + lt);
                  (t = a > 0 ? Math.floor(t) : Math.ceil(t)), (qt += t);
                } else {
                  var e = -(Je + a);
                  if (e <= 0) qt = Vt;
      ...
      O.preventScrollOnTouch === 'auto' && (be = !1),
        Xt && (Yt = '?'),
        Tt && !qe && di();
    }
    function Ni() {
