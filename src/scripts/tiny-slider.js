  ...
  var tns = function(options) {
    ...
    function initTools() {
      ...
      if (hasAutoplay) {
        ...
        if (autoplayButton) {
          ...
          outerWrapper.insertAdjacentHTML(
            options.autoplayPosition !== 'top' ? 'beforeend' : 'afterbegin',
            '<button data-action="' +
              txt +
              '" type="button">' +
      ...
      if (hasNav) {
        ...
        if (navContainer) {
          ...
          navHtml =
            ...
            '</div>';
          outerWrapper.insertAdjacentHTML(
            options.navPosition !== 'top' ? 'beforeend' : 'afterbegin',
            navHtml
          );
      ...
      if (hasControls) {
        if (!controlsContainer && (!prevButton || !nextButton)) {
          outerWrapper.insertAdjacentHTML(
            options.controlsPosition !== 'top' ? 'beforeend' : 'afterbegin',
    ...
    function lazyLoad() {
      if (lazyload && !disable) {
        ...
        for (; i < len; i++) {
          forEachNodeList(slideItems[i].querySelectorAll(lazyloadSelector), function(
            img
          ) {
    ...
    function getSliderWidth() {
      return fixedWidth
        ...
        : slidePositions[slideCountNew - 1] +
            slideItems[slideCountNew - 1].getBoundingClientRect().width;
    }
      ...
      if (carousel) {
        translateInit = parseFloat(
          container.style[transformAttr]
            .replace(transformPrefix, '')
            .replace(transformPostfix, '')
        );
        resetDuration(container, '0s');
      }
    }
      ...
      if (moveDirectionExpected) {
        ...
        if (!horizontal || fixedWidth || autoWidth) {
          ...
          var percentageX = TRANSFORM
            ? (dist * items * 100) / (viewport * slideCountNew)
            : (dist * 100) / viewport;
          x += percentageX;
          x += '%';
        }
    ...
    function getVisibleNavIndex() {
      ...
      if (
        (loop && visibleNavIndexes.length * items < slideCount) ||
